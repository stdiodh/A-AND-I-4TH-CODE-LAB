...
`02`부터는 각 시퀀스마다 메인 구현 주제 외에
주니어 백엔드 개발자가 실무에서 반드시 만나게 되는 확장 개념 1개를 같이 다룹니다.

이 규칙은 다음 시퀀스부터 별도 지시가 없어도 기본값으로 자동 적용합니다.

Codex는 실무 확장 개념이 있는 시퀀스를 작업할 때 아래를 반드시 같이 반영해야 합니다.
...
1. 왜 이 문제가 실무에서 보이는가
2. 처음 보면 자연스러운 문제 코드
3. 내부에서 실제로 무슨 쿼리나 동작이 일어나는가
4. 왜 느려지거나, 위험하거나, 운영이 어려워지는가
5. 대표 해결 코드 예시가 어떤 모양인가
6. 이번 시퀀스에서 어디까지 구현하고 어디까지는 설명만 하는가

즉, 실무 확장 개념은 이름만 소개하지 않습니다.
반드시 문제 코드 + 내부 동작 + 비용 설명 + 해결 코드 예시까지 같이 보여줘야 합니다.

특히 성능, 보안, 운영 주제는
학생이 공감할 수 있도록 “왜 실제로 비용이 커지는지”를 코드 기준으로 설명해야 합니다.

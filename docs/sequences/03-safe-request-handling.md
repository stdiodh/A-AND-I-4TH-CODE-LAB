...
**사용자 입력을 어디까지 믿을 수 있는지**, **실패를 어떤 계약으로 돌려줄지**를
...
**입력을 더 안전하게 받고, 실패를 더 명확하게 응답하는 것**에 집중합니다.
...
- 이번 시퀀스의 **메인 구현 흐름은 기본 `@NotBlank` + `@Valid` + 예외 응답 통일**로 유지합니다.
- 대신 이론 문서에서는 **기본 검증만으로 막히지 않는 문제 입력**을 보여줍니다.
...
즉 `03`의 실무 확장 개념은
**기본 Validation 소개**가 아니라
**기본 검증의 한계와 커스텀 검증의 방향까지 같이 읽는 입문 단계**로 설계합니다.

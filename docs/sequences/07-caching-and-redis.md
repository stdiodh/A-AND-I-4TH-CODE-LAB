...
5. TTL을 설정합니다.
6. 수정/삭제 시 캐시를 무효화합니다.
7. hit/miss/invalidation 로그나 응답 차이를 확인합니다.
...
- 수정 또는 삭제 후 캐시 무효화 흐름
- Redis 컨테이너 상태

## 자주 발생하는 문제

- 캐시가 있으면 DB가 필요 없다고 생각합니다. 캐시는 DB를 대체하지 않습니다.
- 수정 후 캐시를 지우지 않아 오래된 값이 나옵니다. 쓰기 흐름에서는 invalidation을 확인합니다.

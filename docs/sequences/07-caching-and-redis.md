...
7. 수정/삭제 시 필요한 캐시 무효화 전략을 확장 과제로 비교합니다.
...
- 현재 답안은 조회 cache hit/miss와 TTL 저장까지 확인합니다.
- 수정 또는 삭제 후 invalidation은 구현되지 않은 확장 과제로 구분합니다.

실패하면 먼저 볼 것:

- 캐시 key가 저장할 때와 조회할 때 같은 규칙을 쓰는지 확인합니다.
- 캐시 hit/miss를 확인하는 테스트에서 DB 호출 횟수 또는 로그 기준이 명확한지 봅니다.
- invalidation을 확장한다면 수정/삭제 후 이전 캐시 값이 남아 있지 않은지 확인합니다.

완료 기준:

- cache miss 테스트가 통과합니다.
- cache hit 테스트가 통과합니다.
- 현재 구현에 없는 invalidation을 완료된 기능처럼 설명하지 않습니다.
...
- 수정 또는 삭제 후 캐시 무효화가 필요한 이유와 구현 후보
- Redis 컨테이너 상태

## 자주 발생하는 문제

- 캐시가 있으면 DB가 필요 없다고 생각합니다. 캐시는 DB를 대체하지 않습니다.
- 현재 답안은 수정 후 캐시를 지우지 않으므로 stale data가 생길 수 있습니다. 쓰기 흐름을 확장할 때 invalidation을 함께 구현합니다.

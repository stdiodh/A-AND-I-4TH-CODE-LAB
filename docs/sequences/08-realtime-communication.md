...
- 클라이언트가 topic을 구독할 수 있는지 확인합니다.
- 메시지를 발행하면 서버 handler가 처리하는지 확인합니다.
- 구독한 클라이언트가 broadcast 메시지를 수신하는지 확인합니다.

실패하면 먼저 볼 것:

- connect가 완료되기 전에 subscribe나 send를 실행하지 않았는지 확인합니다.
- 구독 경로와 발행 경로가 서로 다른 역할로 설정되어 있는지 봅니다.
- 수신 테스트는 timeout이 너무 짧거나 메시지 타입이 다른지 확인합니다.

완료 기준:

- 연결 테스트가 통과합니다.
- 구독과 메시지 발행 테스트가 통과합니다.
- 수신 확인 테스트가 통과합니다.

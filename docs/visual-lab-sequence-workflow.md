...
- Step Explorer에서 step rail, 현재 단계 카드, 재생/일시정지, 속도 조절, 이전/다음 버튼이 동작한다.
- topic card는 개념 인덱스로 동작하고, 카드 클릭은 상세/핵심/자료 영역만 갱신한다.
- Step Explorer는 `visualLabFocusFlows`에 정의한 핵심 흐름 1~2개만 표시한다.
- 핵심 흐름의 단계는 input/output/handoff를 통해 다음 단계로 무엇을 넘기는지 보여준다.
- 핵심 흐름 선택 탭이 있고, topic card 클릭으로 Step Explorer가 카드별 단계로 바뀌지 않는다.

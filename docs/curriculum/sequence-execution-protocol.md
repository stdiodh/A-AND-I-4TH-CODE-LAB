...
5. 바로 이전 시퀀스의 `docs/sequences/0Y-...md`

현재 시퀀스 외 작업은 하지 않습니다.

여기서 이전 시퀀스를 읽는 이유는
현재 문서가 이어받는 기초 용어와 흐름이 무엇인지 확인하기 위해서입니다.
...
- 이 규칙이 `기초 이론 설명을 줄여도 된다`는 뜻은 아닙니다.
- 실무 확장 개념은 언제나 `기초 개념 설명`과 `현재 코드 흐름 설명` 뒤에 와야 합니다.
- 만약 `theory.md`가 실무 확장 개념만 길고, 기본 기술 설명이 사라졌다면 실패입니다.

즉, `theory.md`는 아래 3층을 모두 만족해야 합니다.
...
### Step 6-3. 기초 설명이 빠진 이론 문서는 미완성으로 봅니다

아래 중 하나라도 해당하면 그 이론 문서는 완료된 것으로 보지 않습니다.

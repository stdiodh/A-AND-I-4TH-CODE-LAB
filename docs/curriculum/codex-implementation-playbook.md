...
11. 바로 이전 시퀀스의 `docs/sequences/0Y-...md`
12. 그 다음 해당 토픽 레포의 `README.md`, `docs/theory.md`, `docs/implementation.md`, `docs/checklist.md`, starter 코드
...
5. 이전 시퀀스와의 연결 지점 확인
6. 해당 도메인 문서 확인
7. 실제 레포 구조와 starter 코드 확인
8. TODO 설계 및 문서/코드 생성
...
4. 이전 시퀀스에 있던 필수 용어가 이번 문서에서 빈칸처럼 사라지지 않았는가

이 질문 중 하나라도 `아니오`면,
문서는 다시 써야 합니다.

### 6-3. 이전 시퀀스를 보고 기초 설명 누락을 점검합니다

Codex는 현재 시퀀스 문서를 쓸 때
바로 이전 시퀀스 문서를 반드시 함께 봐야 합니다.

이때 확인할 것은 "이전 시퀀스 내용을 그대로 끌고 올 것인가"가 아니라,
"이번 시퀀스 이해에 여전히 필요한 기초 용어와 연결 흐름이 무엇인가"입니다.

예:

- `04`에서 JWT를 설명했더라도, `05`에서 OAuth2와 JWT 연결이 필요하면 짧게 다시 설명
- `04`에서 인증을 설명했더라도, `05`에서 provider / providerId / OAuth login은 다시 설명

즉, 이전 시퀀스 문서를 보는 목적은 복붙이 아니라
기초 설명 누락 방지입니다.

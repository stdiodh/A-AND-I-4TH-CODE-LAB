...
상태: 06 원격 전환 완료, Visual Lab·중앙 반영 대기
...
두 공식 06 원격 브랜치는 atomic push와 exact lease로 위 후보에 전환됐고 fetch 뒤 SHA와 최신 05 ancestry를 확인했다. Visual Lab main 후보는 [draft PR #18](https://github.com/stdiodh/spring-boot-db-access-lab/pull/18)로 분리해 아직 `main`에는 합치지 않았다.
...
후속 반영은 Visual Lab draft PR 검토·병합, 중앙 submodule pointer 갱신, 중앙 validator 실행 순서로 진행한다. 중앙 root 브랜치는 아직 commit하거나 push하지 않았다.
...
- Visual Lab draft PR을 검토·병합한 뒤 중앙 submodule pointer를 반영한다.
- 중앙 validator 결과와 최종 remote SHA를 기록한다.
- 이 중앙 문서 변경을 root 작업 브랜치에 commit·push한다.

위 항목까지 완료되고 07과 다른 시퀀스에 변경이 없을 때 상태를 `완료`로 바꾼다.

...
| 07 | Redis Cache | `spring-boot-redis-cache-lab` | `07-implementation` | `07-answer` | 확인 필요 | manifest상 `needs-visual-lab` | Visual Lab 미완성 | 4 |
...
| 11 | Refactoring Foundation | `spring-boot-refactoring-foundation-lab` | `11-implementation` | `11-answer` | 확인 필요 | manifest상 `needs-review` | 이전 감사에서 10 CI/CD 내용 혼입 의심 | 3 |
| 12 | Event Driven | `spring-boot-event-driven-lab` | `12-implementation` | `12-answer` | 확인 필요 | manifest상 `needs-visual-lab` | 원격 HEAD가 `12-answer`를 가리킬 수 있음 | 2 |
...
| `spring-boot-redis-cache-lab` | manifest상 Visual Lab 미완성 구간입니다. |
...
| `spring-boot-refactoring-foundation-lab` | manifest notes 기준 main 문서에 10 CI/CD 내용 혼입 가능성이 있어 우선 감사가 필요합니다. |
| `spring-boot-event-driven-lab` | manifest notes 기준 원격 default branch가 `12-answer`를 가리킬 가능성이 있어 GitHub UI 수동 조치가 필요합니다. |
...
| `spring-boot-redis-cache-lab` | manifest상 `needs-visual-lab`. |
...
| `spring-boot-refactoring-foundation-lab` | manifest상 `needs-review`. |
| `spring-boot-event-driven-lab` | manifest상 `needs-visual-lab`. |
...
3. SEQ 12 Event Driven: 원격 HEAD/default branch 리스크와 Visual Lab 미완성을 다음 우선 감사 대상으로 둡니다.
4. SEQ 11 Refactoring Foundation: 10 CI/CD 내용 혼입 의심을 다음 우선 감사 대상으로 둡니다.
5. SEQ 07~10: Visual Lab 미완성 구간을 중앙 스펙에 맞춰 순차 정리합니다.
...
- SEQ 02~12 각 implementation 브랜치의 정답 노출 검색과 문서 보강은 아직 수행하지 않았습니다.
- SEQ 07~12 Visual Lab 신규 구현 또는 보강은 아직 수행하지 않았습니다.
- legacy 브랜치 삭제, default branch 변경, 원격 branch 정리는 수행하지 않았습니다. 이는 GitHub UI 또는 명시적 승인 후 처리해야 합니다.
- 중앙 레포 push는 루트 커밋 생성 후 수행해야 합니다.

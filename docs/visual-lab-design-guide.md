...
Visual Lab은 글보다 흐름을 먼저 보여준다.
Topic card는 개념 인덱스 역할을 맡고, Step Explorer는 해당 시퀀스의 핵심 흐름 1~2개만 보여준다.
모든 topic card에 개별 step을 붙이지 않는다.
...
- 기존 밝은 배경과 네이비/블루/청록 팔레트를 유지한다.
- active step은 청록 테두리와 연한 민트 배경으로 표현한다.
- 지나간 step은 연한 블루 배경으로만 표시한다.
- 현재 카드는 연한 블루/화이트 패널로 둔다.
- 전체 카드를 진한 색으로 뒤집지 않는다.
- 다크 코드 에디터 스타일을 사용하지 않는다.
- 애니메이션은 단계 전환, 진행선, 데이터 이동에만 쓴다.
- 모바일에서는 rail과 그림을 가로 스크롤로 둔다.
